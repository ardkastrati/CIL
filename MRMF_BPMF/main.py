...
def nmf():
    ...
    train_new_format = "train_new_format.csv"
    file_path = os.path.expanduser('~/.surprise_data/' + train_new_format)
    algo = NMF(verbose=True, biased=True, n_epochs=30, n_factors=1, reg_pu=0.3, reg_qi=0.3, reg_bu=0.02, reg_bi=0.02,
               lr_bu=0.005, lr_bi=0.005)
    train_data = Dataset.load_from_file(file_path, reader=reader)
    trainset = train_data.build_full_trainset()
    algo.fit(trainset)
    IOHelper.surprise_output_submission(algo, 'nmf_biased.csv', verbose=True)
...
def svd():
    ...
    print(trainset)
    algo.fit(trainset)
    IOHelper.surprise_output_submission(algo, 'svd.csv', verbose=True)
...
def main():
    ...
    # MAKE IT NOW WITH BPMF - NO IMPLICIT DATA, NO BIAS! (File real_bpmf.py) - Unfortunately, real_bpmrmf.py has slightly different output.
